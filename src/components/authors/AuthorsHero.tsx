'use client';

import { useTranslation } from 'react-i18next';
import HeroPattern from '@/components/home/HeroPattern';

interface AuthorsHeroProps {
  totalAuthors: number | null;
  totalBooks: number | null;
  localeTag: string;
  onBrowse?: () => void;
}

const AuthorsHero = ({ totalAuthors, totalBooks, localeTag, onBrowse }: AuthorsHeroProps) => {
  const { t } = useTranslation();

  const authorsDisplay =
    totalAuthors === null ? '…' : totalAuthors.toLocaleString(localeTag);
  const booksDisplay = totalBooks === null ? '…' : totalBooks.toLocaleString(localeTag);

  return (
    <section className="relative overflow-hidden rounded-editorial border border-[var(--border)] bg-[var(--bg-elev)] p-7 md:p-8 shadow-soft">
      <HeroPattern />
      <p className="relative mb-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-accent">
        {t('authors.heroEyebrow')}
      </p>
      <h1 className="font-display relative text-3xl font-medium leading-[1.05] tracking-tight text-ink md:text-[2.375rem]">
        {t('authors.heroTitleLine1')}
        <br />
        <em className="font-normal not-italic text-accent">{t('authors.heroTitleEm')}</em>
      </h1>
      <p className="relative mt-3 max-w-xl text-sm leading-relaxed text-ink-muted md:text-[15px]">
        {t('authors.pageDescription')}
      </p>
      <div className="relative mt-5 flex flex-wrap gap-7">
        <div>
          <span className="font-display block text-[22px] font-semibold leading-none tracking-tight text-ink tabular-nums">
            {authorsDisplay}
          </span>
          <span className="mt-1 block text-[11px] tracking-wide text-ink-muted">
            {t('authors.totalAuthors')}
          </span>
        </div>
        <div>
          <span className="font-display block text-[22px] font-semibold leading-none tracking-tight text-ink tabular-nums">
            {booksDisplay}
          </span>
          <span className="mt-1 block text-[11px] tracking-wide text-ink-muted">
            {t('authors.totalBooks')}
          </span>
        </div>
      </div>
      {onBrowse && (
        <button
          type="button"
          onClick={onBrowse}
          className="relative mt-6 inline-flex h-11 items-center rounded-full bg-ink px-5 text-sm font-semibold text-cream transition-transform hover:-translate-y-px"
        >
          {t('authors.browseByLetter')}
        </button>
      )}
    </section>
  );
};

export default AuthorsHero;
